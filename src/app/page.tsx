  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        setError('faileid to fetch products try again');
      ...
      <main className="flex-1 pt-20">
        ...
        <section className="bg-green-500 py-16 px-6 text-white flex flex-col items-center gap-10 md:py-10 md:px-4">
          <div className="text-center max-w-[600px]">
            <h1 className="text-4xl md:text-3xl font-extrabold mb-3 leading-tight">
        ...
        <section className="py-8 px-6 bg-white md:py-6 md:px-4">
          <h2 className="text-2xl font-bold text-[#1a1a1a] mb-5">Shop by Category</h2>
          <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-thin">
            {categories.map((category) => (
              <button
                key={category.id}
                onClick={() => setSelectedCategory(category.id)}
                className={`flex flex-col items-center gap-2 py-4 px-6 bg-[#f5f5f5] border-2 border-transparent rounded-xl cursor-pointer transition-all duration-200 min-w-[120px] flex-shrink-0 md:py-3 md:px-4 md:min-w-[100px] hover:bg-[#e8f5e9] hover:border-[#0c831f] ${selectedCategory === category.id
